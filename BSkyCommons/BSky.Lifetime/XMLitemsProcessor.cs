...
{
    //This is a generic class and it can be used to read any xml that has structure like "DefaultPackages.xml"
    //This class may need some cleaning as it may have some extra code that may not be required at all.
    //We need to drop DefaultPackages.cs as the of that class is not so generic. This is same code but name is generic
    //and can be used in several situations without confusion.
    public class XMLitemsProcessor
    {
